...
def display_resume_grid(resumes, container=None):
    ...
    # Highlighted terms for matching - ONLY exact search terms
    search_terms = ['python', 'sql']  # Only highlight actual search terms, not all variants
            ...
            if idx < num_resumes:
                ...
                with cols[col]:
                    ...
                    if skills:
                        ...
                        # Show ALL skills with highlighting ONLY for exact search terms
                        for skill in skills:
                            skill_name = skill if isinstance(skill, str) else skill.get('skillName', '')
                            # Check if this skill matches EXACT search terms
                            is_highlight = any(term.lower() == skill_name.lower() for term in search_terms)
                    ...
                    if keywords:
                        ...
                        # Show ALL keywords with highlighting ONLY for exact search terms
                        for keyword in keywords:
                            # Check if this keyword matches EXACT search terms
                            is_highlight = any(term.lower() == keyword.lower() for term in search_terms)
                            highlight_class = ' highlight' if is_highlight else ''
